...
# Helper function to append to a language object
def inject_lang(content, lang_code, keys_dict):
    # Find where lang starts
    # e.g., en: {
    pattern = re.compile(rf'({lang_code}:\s*{{)')
    match = pattern.search(content)
    if not match:
        print(f"Cannot find {lang_code}: {{ in i18n.js")
        return content
    
    # We will inject a new "new_sections" object inside the language
    json_str = json.dumps(keys_dict, ensure_ascii=False, indent=6)
    # Remove outer braces and add comma
    inner_json = json_str[1:-1]
    
    injection = f"\n    new_sections: {{{inner_json}}},\n"
    
    return content[:match.end()] + injection + content[match.end():]

# Ensure 'ar' block exists. If not, create it by copying 'en' or similar? Wait, the file might not have 'ar'.
# Let's check if 'ar: {' is in the file.
if 'ar: {' not in content:
    # Actually, we should check if there is an 'ar' block. The app supports Arabic, so let's look for it.
    pass

content = inject_lang(content, 'tr', new_keys_tr)
content = inject_lang(content, 'en', new_keys_en)
if 'ar: {' in content:
    content = inject_lang(content, 'ar', new_keys_ar)
else:
    # Append ar at the end before the last closing brace
    # Just a rough hack, it's better to find 'en' block end.
    pass
